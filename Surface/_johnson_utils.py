"""
utilities used by slippy.surfecs
...
from scipy.stats._continuous_distns import _norm_pdf, _norm_cdf, _norm_ppf
...
def _fit_johnson_by_moments(mean, sd, root_beta_1, beta_2, return_rv=True):
    ...
    def returnsl(mean, sd, root_beta_1, omega, return_rv):
    ...
    if abs(x)<tollerance:
        return returnsl(mean, sd, root_beta_1, omega, return_rv)
    ...
    if x>0:
        try:
            ...
            warnings.warn("SB fit itterations failed returning SL distribution")
            print(omega)
            return returnsl(mean, sd, root_beta_1, omega, return_rv)
...
def _fit_johnson_by_quantiles(quantiles):
    ...
    quntiles relate to normal quanties of -1.5, -0.5, 0.5, 1.5
