...
class UnifiedMixedSolver():
    ...
    def __init__(self, surface1, surface2, normal_load=None):
...
def EHL(N,N1):
#****************** Main subroutine that integrates EHL equations to get solution: START ***************
    I,J,K,K1,K2,MK,KK,MM,ICB,ICM,ICP,kball=int(0)
    PAI=3.14159265
    Z=0.68
    P0=1.96E8
    PH,E1,EDA0,RX,US,X0,XE,CU=float(0)
    DT0,DT,G0,DAB,DAD=float(0)
    X,Y = np.zeros((N,1))
    ...
    A1,A2,A3,U,B,W0,W,ALFA,G,HM0,ENDA,U1,U2=float(0)
    FM,P2,H2,P3,H3,DX,RMSR,ER,FRICTION,RMSRB,DH,H00,ERH,G001=float(0)
#    REAL (kind=8) :: DH,H00,ERH
    DF,MEANBALLR,MEANDISCR,MEANBALLT,MEANDISCT,MEANBALLW,MEANDISCW,RMSB,RMSD=float(0)
    UR,HM0AVG,SUM1,H00OLD,SUM2,YmeanD,YmeanB,MEANDISCP,MEANBALLP=float(0)
    COUNT,COUNT1,COUNT2,nx,ny,nh=int(0)
    TT,TT0,DW,Wrel=float(0)
    LBOUT,LASPT,LBOUS,LASPS=float(0)
    HMAX,BOLTZZ,PLANK,XTRIBO,TB,A,Cthree,Cfour ,Hsub,Hfilm,Hardness=float(0)
    TLOCALB,TLOCALD,TLOCALBFORM,TLOCALBREM,TLOCALDFORM,TLOCALDREM =float(0)
    HMAX2,Cfive,Csix,UPLASTIC,UPLA,G00temptest,converg=float(0)
    Kmax,Kmin,Ktrb,Ktrd,WEARSB,WEARSD,HMC,H00initial,G00,G00OLD,PLANE1=float(0)
    U1temp=np.zeros((15,1))
    C_GE,HMAX_GE,EACT_GE,VACT_GE,MAX_DISC,MAX_BALL,TFB,TFD=float(0)
...
#****************** Main subroutine that integrates EHL equations to get solution: END ***************    
    RETURN
    END
#**********************************************************************
    SUBROUTINE ERP(N,W,ER,P,POLD)
    IMPLICIT NONE
...
10  CONTINUE
    ...
    END SUBROUTINE ERP
#**********************************************************************
    SUBROUTINE GEOM(N,DAB,X,Y,GOU,ROU2,RMSRB)
    IMPLICIT NONE
    INTEGER :: N,I,J
...
# ******************************* #
    ...
    END SUBROUTINE FZ
# ******************************* #
    SUBROUTINE ROUGHNESS(N,DA,ROU,RMSA)
...
#******************************************#
    SUBROUTINE ITERSEMISEPI(N,KK,DX,DT,ERH,H00,G0,X,Y,H,HO0,RO,RO0,EPS,EDA,P,V,GOU,ROU)
    IMPLICIT NONE
...
54      IF(P(I,J).LT.0.0)P(I,J)=0.0
    ...
    RETURN
    END SUBROUTINE ITERSEMISEPI
#**********************************************************************
    SUBROUTINE HREEI(N,DX,DF,DT,ERH,KK,H00,G0,X,Y,H,RO,EPS,EDA,P,V,GOU,ROU)
    IMPLICIT NONE
