...
def elastic_displacement(deflections: dict, grid_spacing: tuple, shear_modulus: float,
    ...
    shear_modulus : float optional (None)
    ...
    if components is None:
        ...
        components = elastic_im(grid_spacing, span, shear_modulus, v, comp_names)
    else:
...
def _solve_ed(deflections: dict, components: dict, max_it: int, tol: float):
    ...
    # start loop
    d = r
    deflections = {'x': np.zeros_like(deflections[def_directions[0]]),
                   'y': np.zeros_like(deflections[def_directions[0]]),
                   'z': np.zeros_like(deflections[def_directions[0]])}
    ...
    while resid_norm > tol:
        ...
        for dd in def_directions:
            ...
            deflections[dd][sub_domains[dd]] = d[start:end]
            start = end

        # find z (equation 25 in ref)
        calc_deflections = _solve_el(deflections, components)
        z = np.array([])
        for dd in def_directions:
...
def elastic_loading(loads: dict, grid_spacing: tuple, v: float, shear_modulus: float,
...
def _elastic_im_getter(k, l, m, n, hx, hy, shear_mod, v, comp) -> np.array:
    ...
    K^{ij i'j'}_zz=(1-v)/(2*pi*G)*stiff_zz
...
def combined_modulus(stiff_1: float, v_1: float, stiff_2: float = None,
                     v_2: float = None, modulus: str = 'elastic') -> float:
    """Find the combined elastic or shear modulus of a material pair
    
    Parameters
    ----------
    stiff_1,stiff_2 : float
        The stiffness or youngs modulus of the materials
    v_1,v_2 : float
        The Poisson's ratio of the materials
    modulus : str optional {'elastic', 'shear'}
    ...
    if modulus == 'elastic':
        return 1 / ((1 - v_1 ** 2) / stiff_1 + (1 - v_2 ** 2) / stiff_2)
    elif modulus == 'shear':
        return 1 / ((1 + v_1) * (1 - 2 * v_2) / 2 / stiff_1 - (1 + v_2) * (1 - 2 * v_1) / 2 / stiff_2)
    else:
        msg = ("Unrecognised modulus '{}' supported types are: ".format(modulus)
...
if __name__ == '__main__':
    ...
    loads1 = s.RoundSurface(1)
    loads1.extent = [5.11, 5.11]
    loads1.grid_spacing = 0.01
    loads1.descretise()

    loads1 = np.array(loads1) * 1000
